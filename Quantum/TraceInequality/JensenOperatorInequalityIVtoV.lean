...
-- Local `synthInstance.maxHeartbeats` for the next `inferInstance`: `NonnegSpectrumClass ℝ (L (HSum ℋ))`
-- pulls a long C*-algebra / `WithLp`-related instance search.
...
set_option synthInstance.maxHeartbeats 400000 in
-- `StarAlgHom.map_cfc` / `IsSelfAdjoint.map` on `L (HSum ℋ)` needs a deeper typeclass search.
omit [Nontrivial ℋ] in
