...
-- Local `maxHeartbeats` for the declaration below: `cfcR` / `CFC.rpow_*` rewrites and `simpa`
-- cost many elaboration/defeq heartbeats on `L ℋ`, not only CFC instance synthesis.
omit [FiniteDimensional ℂ ℋ] in
...
private lemma liebTraceMap_mono_right
  ...
  have hsplit :
    ...
    ext T
    change (A ^ s * (toOp T * (B₂ ^ (1 - s) - B₁ ^ (1 - s))) : L ℋ) =
...
theorem andoTrace_jointlyConvexOn_pdSet
  ...
  · have hrz : r = 0 := by linarith
    subst hqeq
    subst hrz
    convert (liebTrace_jointlyConvexOn_pdSet (ℋ := ℋ)
      (s := 1) (by norm_num) (by norm_num) K) using 1
    ext A B
