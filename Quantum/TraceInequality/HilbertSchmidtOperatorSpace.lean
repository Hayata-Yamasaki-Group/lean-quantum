...
instance : FiniteDimensional ℂ (HSOp ℋ) := by
  change FiniteDimensional ℂ (ℋ →L[ℂ] ℋ)
  infer_instance
...
noncomputable def rightMulHSStarAlgHom : (L ℋ)ᵐᵒᵖ →⋆ₐ[ℝ] L (HSOp ℋ) where
  ...
  commutes' := by
    intro r
    change rightMulHS (ℋ := ℋ) (MulOpposite.unop (MulOpposite.op (r • (1 : L ℋ)))) =
      r • (1 : L (HSOp ℋ))
...
private noncomputable def opStarHSLinearMap : L ℋ →ₗ[ℝ] (L ℋ)ᵐᵒᵖ where
  ...
  map_smul' r A := by
    ...
    change star A * star ((algebraMap ℝ (L ℋ)) r) = (algebraMap ℝ (L ℋ)) r * star A
...
private noncomputable def leftMulHSLinearMap : L ℋ →ₗ[ℝ] L (HSOp ℋ) where
  ...
  map_smul' r A := by
    ext T
    change ((r • A) * toOp T : L ℋ) = r • (A * toOp T)
    rw [smul_mul_assoc]
